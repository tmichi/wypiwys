...
#ifdef VM_INCLUDE_IO
#  ifdef VM_USE_OLDIOSTREAM // <iostream.h> or <iostream>
#    include <iostream.h>
#  else
#    include <iostream>
#  endif
...
#     include <string>
#     ifdef VM_USE_STRINGSTREAM // <stringstrem> or <strstream.h> or <sstream>
#       include <stringstream>
#       define VM_TOSTRING VM_STRING_STD::stringstream s; s << *this; return s.str();
#     elif defined(VM_USE_OLDIOSTREAM)
#       if defined(_MSC_VER)
#         include <strstrea.h>
...
/**
 * Helper class for dynamic memory allocation in GMatrix/GVector
 */
#ifndef VM_USE_SPECIAL_ALLOCATOR
template<class T>
class GMVallocator {
    typedef typename VM_STD::allocator<T> alloc_t;
    typedef typename VM_STD::allocator<T*> palloc_t;
public:
    inline T *allocate(size_t n) {
        static alloc_t alloc;
        return alloc.allocate(n, NULL);
    }
    ...
    inline T **p_allocate(size_t n) {
        static palloc_t palloc;
        return palloc.allocate(n, NULL);
    }
    inline void p_deallocate(T **p, size_t n) {
        static palloc_t palloc;
        if (p) {palloc.deallocate(p, n);}
    }
};
#endif
